 ...
 */
use application\models\Entities\entities_wheat\E_Wheat_InternalFort_B1;

class M_Wheat_InternalFort_B1  extends MY_Model {
	...
	function addRecord() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			$manufacturer=$this->em->getRepository('models\Entities\E_ManufacturerCompound')
			 ...
			 $this -> theForm = new \models\Entities\entities_wheat\E_Wheat_InternalFort_B1(); //create an object of the model
				...
				$this -> theForm -> setTheoreticalFeederFlow($this->elements[$i]['theoreticFeeder']);
				...
				$this -> theForm -> setTransactedBy($this->input->post('personResponsible'));

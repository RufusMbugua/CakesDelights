 ...
 */
use application\models\Entities\entities_wheat\E_Wheat_InternalFort_A1;

class M_Wheat_InternalFort_A1  extends MY_Model {
		...
		if ($this -> input -> post()) {//check if a post was made
		  ...
		  // print(($key." ".$val)).'<br \>';
			...
			$premix=$this->em->getRepository('models\Entities\E_PremixType')
			 ...
			 $this -> theForm = new \models\Entities\entities_wheat\E_Wheat_InternalFort_A1(); //create an object of the model
				...
				$this -> theForm ->setProductionObservation($this->elements['productObservation']);
				$this -> theForm ->setExpiryDate($this->elements['expirationDate']);
				$this -> theForm ->setExpiryObservation($this->elements['expirationObservation']);
				$this -> theForm ->setMicronutrientLevelsInLabel($this->elements['contentClaimed']);
				$this -> theForm ->setMicronutrientObservation($this->elements['contentObservation']);
				$this -> theForm ->setCertificateOfAnalysis($this->elements['certOfanalysis']);
				$this -> theForm ->setCertificateOfAnalysisObservation($this->elements['certificateObservation']);

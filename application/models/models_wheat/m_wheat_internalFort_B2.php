 ...
 */
use application\models\Entities\entities_wheat\E_Wheat_InternalFort_B2;

class M_Wheat_InternalFort_B2  extends MY_Model {
	...
	function addRecord() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			$manufacturer=$this->em->getRepository('models\Entities\E_ManufacturerCompound')
			 ...
			 $this -> theForm = new \models\Entities\entities_wheat\E_Wheat_InternalFort_B2(); //create an object of the model
		      
			 	
				$this -> theForm -> setDates($this->input->post('dateB2'));
				$this -> theForm -> setShiftTime($this->elements[$i]['shiftTime']);
				$this -> theForm -> setFlourProduced($this->elements[$i]['flourMT']);

 ...
 */
use application\models\Entities\entities_wheat\E_Wheat_InternalFort_A2;

class M_Wheat_InternalFort_A2  extends MY_Model {
	...
	function addRecord() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			$manufacturer=$this->em->getRepository('models\Entities\E_ManufacturerCompound')
			 ...
			 $this -> theForm = new \models\Entities\entities_wheat\E_Wheat_InternalFort_A2(); //create an object of the model
		      
			 	
				$this -> theForm -> setDates($this->elements[$i]['theDate']);
				$this -> theForm -> setShiftTime($this->elements[$i]['theTime']);
				$this -> theForm -> setQuantity($this->elements[$i]["quantity"]);

 ...
 */
use application\models\Entities\entities_wheat\E_Wheat_InternalFort_C1;

class M_Wheat_InternalFort_C1  extends MY_Model {
		...
		if ($this -> input -> post()) {//check if a post was made
		  ...
		  // print(($key." ".$val)).'<br \>';
			...
			//iodization centre name obtained from the session variable => 'affiliation'
			 ...
			 $this -> theForm = new \models\Entities\entities_wheat\E_Wheat_InternalFort_C1(); //create an object of the model
				...
				$this -> theForm -> setWheatProduced($this->elements[$i]["wheatFlourProduced"]);
				$this -> theForm -> setPremixUsed($this->elements[$i]["premixUsed"]);

				$this -> theForm -> setWheatFlourVsPremix($this->elements[$i]["wheatFlour"]);
				$this -> theForm -> setNotes($this->elements[$i]["notes"]);
				$this -> theForm -> setComments($this->elements[$i]["comments"]);
				$this -> theForm -> setResponsible($this->input->post("responsible"));

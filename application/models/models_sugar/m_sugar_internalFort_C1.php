 ...
 */
use application\models\Entities\entities_sugar\E_Sugar_InternalFort_C1;

class M_Sugar_InternalFort_C1  extends MY_Model {
	...
	function addRecord() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			$manufacturer=$this->em->getRepository('models\Entities\E_ManufacturerCompound')
			 ...
			 $this -> theForm = new \models\Entities\entities_sugar\E_Sugar_InternalFort_C1(); //create an object of the model
		      
			 	
				$this -> theForm -> setDates($this->elements[$i]['dateReceived']);
				
				$this -> theForm -> setPremixType($this->elements[$i]['ingredients']);
				$this -> theForm -> setQuantity($this->elements[$i]['quantityReceived']);
				$this -> theForm -> setLotNumber($this->elements[$i]['lotNumber']);
				$this -> theForm -> setExpirationDate($this->elements[$i]['expirationDate']);
				$this -> theForm -> setCOA($this->elements[$i]['certificateOfAnalysis']);
				$this -> theForm -> setReceivedBy($this->elements[$i]['receivedBy']);				

 ...
 */
use application\models\Entities\entities_sugar\E_Sugar_InternalFort_D1;

class M_Sugar_InternalFort_D1  extends MY_Model {
	...
	function addRecord() {
		...
		if ($this -> input -> post()) {//check if a post was made
			...
			$manufacturer=$this->em->getRepository('models\Entities\E_ManufacturerCompound')
			 ...
			 $this -> theForm = new \models\Entities\entities_sugar\E_Sugar_InternalFort_D1(); //create an object of the model
		      
			 	
				
				$this -> theForm -> setLotsID($this->elements[$i]['bagNumbers']);
				$this -> theForm -> setProductionDate($this->elements[$i]['productionDate']);
				$this -> theForm -> setSamplesDate($this->elements[$i]['samplesSent']);
				$this -> theForm -> setLaboratoryReport($this->elements[$i]['laboratoryReport']);
				$this -> theForm -> setReportDate($this->elements[$i]['dateReported']);
				$this -> theForm -> setVitaminA($this->elements[$i]['vitaminA']);
				$this -> theForm -> setObservations($this->elements[$i]["vitaminAObservations"]);

 ...
 */
use application\models\Entities\entities_oil\E_Oil_C1;

class M_Oil_FortifiedC1  extends MY_Model {
		...
		if ($this -> input -> post()) {//check if a post was made
		   ...
		   //print(($key." ".$val)).'<br \>';
			...
			//iodization centre name obtained from the session variable => 'affiliation'
			 ...
			 $this -> theForm = new \models\Entities\entities_oil\E_Oil_C1(); //create an object of the model
				...
				$this -> theForm -> setOilProduced($this->elements[$i]["oilProduced"]);
				$this -> theForm -> setPremixUsed($this->elements[$i]["premixUsed"]);

				$this -> theForm -> setOilFortified($this->elements[$i]["oilFortified"]);
